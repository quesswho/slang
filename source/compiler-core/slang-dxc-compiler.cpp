...
#else
#define __uuidof(x) __mingw_uuidof<StripSlangComPtr<std::decay_t<decltype(x)>>::type>()
// We need to define the uuids in this way, as we can not convert the CROSS_PLATFORM_UUIDOF macro in dxcapi.h
...
__CRT_UUID_DECL(IDxcCompiler, 0x73EFFE2A, 0x70DC, 0x45F8, 0x96, 0x90, 0xEF, 0xF6, 0x4C, 0x02, 0x42, 0x9D);
